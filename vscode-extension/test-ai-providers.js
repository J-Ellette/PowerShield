...
const TemplateBasedProvider_1 = require("../src/ai/TemplateBasedProvider");
/**
 * Create a sample violation for testing
 */
function createSampleViolation(ruleId, code) {
    return {
        name: ruleId,
        message: `Security violation: ${ruleId}`,
        description: `Test violation for ${ruleId}`,
        severity: types_1.SecuritySeverity.High,
        lineNumber: 10,
        columnNumber: 0,
        code: code,
        filePath: 'test.ps1',
 ...
 */
function createSampleContext(code) {
...
async function testTemplateProvider() {
    ...
    const provider = new TemplateBasedProvider_1.TemplateBasedProvider();
    ...
    const hashViolation = createSampleViolation('InsecureHashAlgorithm', '$hash = [System.Security.Cryptography.MD5]::Create()');
    ...
    const credViolation = createSampleViolation('CredentialExposure', '$password = "MyPassword123"');
    ...
    const cmdViolation = createSampleViolation('CommandInjection', 'Invoke-Expression $userInput');
    ...
    const certViolation = createSampleViolation('CertificateValidation', '[System.Net.ServicePointManager]::ServerCertificateValidationCallback = {$true}');
...
async function testProviderAvailability() {
    ...
    const template = new TemplateBasedProvider_1.TemplateBasedProvider();
